...
@interface JXQScrollView ()<UIScrollViewDelegate, UITableViewDelegate, UITableViewDataSource  >
{
...
- (void)initSecondView: (int)i {
    if (i == 0) {
        ...
        _emptyNoUsedLabel.text = @"没有未使用的加息券!";
        ...
        _emptyUsedLabel.text = @"没有已使用的加息券!";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView.tag == 0) {//未使用
        ...
        cell.money.text = [NSString stringWithFormat:@"%.1f", [dataSourceNOUse[indexPath.row][@"voucherValue"] doubleValue] / 10];
...
//        cell.label3.text = [NSString stringWithFormat:@"%@天及以上标的使用", dataSourceNOUse[indexPath.row][@"restricta"]];
        ...
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.money.text = [NSString stringWithFormat:@"%.1f", [dataSourceUsed[indexPath.row][@"voucherValue"] doubleValue] / 10];
...
//        cell.label3.text = [NSString stringWithFormat:@"%@天及以上标的使用", dataSourceUsed[indexPath.row][@"restricta"]];
        ...
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.money.text = [NSString stringWithFormat:@"%.1f", [dataSourceYGQ[indexPath.row][@"voucherValue"] doubleValue] / 10];
